...
const requirements: RequirementDatabase = {
  CCSF: {
    "UC Berkeley": {
      Economics: {
        notes:
...
export default function Home() {
  ...
  const collegeOptions = Object.keys(requirements);

  const selectedCollegeData = communityCollege
    ? requirements[communityCollege] ?? {}
    : {};

  const schoolOptions = Object.keys(selectedCollegeData);

  const selectedSchoolData = targetSchool
    ? selectedCollegeData[targetSchool] ?? {}
    : {};

  const majorOptions = Object.keys(selectedSchoolData);

  const selectedPlan = targetMajor ? selectedSchoolData[targetMajor] : null;

  const previewPlan = requirements.CCSF["UC Berkeley"].Economics;
